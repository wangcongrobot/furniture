...
class FurnitureEnv(metaclass=EnvMeta):
    ...
    def __init__(self, config):
        ...
        self._unity = None
        if config.unity:
            self._unity = UnityInterface(config.port, config.unity_editor,
    ...
    def _step(self, a):
        ...
        if self._connected_body1 is not None:
            self.sim.forward()
            self._move_objects_target(self._connected_body1,
                ...
                if site1_pairs == site2_pairs[::-1]:
                    if self._is_aligned(site1_name, site2_name):
                        ...
                        if self._connect_step < self._num_connect_steps:
                            ...
                            body_pos, body_rot = T.transform_to_target_quat(site2_pos_quat, part2_qpos, site1_quat)
                            ...
                            if self._connect_step == 0:
                                ...
                                xnew = np.linspace(1/self._num_connect_steps, 0.9, self._num_connect_steps)
                                self.next_pos = f(xnew)

                            next_pos, next_rotation = self.next_pos[self._connect_step], self.next_rot[self._connect_step]
                            self._move_objects_target(part2, next_pos, list(next_rotation))
    ...
    def _is_aligned(self, connector1, connector2):
        ...
        if pos_dist < self._env_config['pos_dist'] and \
                ...
                is_rot_forward_aligned and \
                abs(project1_2) > self._env_config['project_dist'] and \
                abs(project2_1) > self._env_config['project_dist']:
            return True

        # connect two parts if they are very close to each other
        if pos_dist < 0.03 and rot_dist_up > self._env_config['rot_dist_up'] and \
    ...
    def _step_continuous(self, action):
        ...
        if self._control_type == 'ik':
            ...
            if self._agent_type == "Sawyer":
                ...
                self._initial_right_hand_quat = T.euler_to_quat(action[3:6] * self._rotate_speed,
                                                                self._initial_right_hand_quat)
                right_d_quat = T.quat_multiply(T.quat_inverse(self._right_hand_quat),
                                               self._initial_right_hand_quat)
                ...
                self._initial_left_hand_quat = T.euler_to_quat(action[9:12] * self._rotate_speed,
                                                               self._initial_left_hand_quat)
                left_d_quat = T.quat_multiply(T.quat_inverse(self._left_hand_quat),
                                              self._initial_left_hand_quat)
    ...
    def _reset(self, furniture_id=None, background=None):
        ...
        for geom_id, body_id in enumerate(self.sim.model.geom_bodyid):
            ...
            if body_name not in self._object_names and \
                    self.mujoco_robot.is_robot_part(geom_name):
                robot_col[geom_name] = (self.sim.model.geom_contype[geom_id],
    ...
    def _initialize_robot_pos(self):
        ...
        if self._agent_type == 'Sawyer':
            ...
            self.sim.data.qpos[self._ref_gripper_left_joint_pos_indexes] = self.gripper_left.init_qpos
    ...
    def run_manual(self, config):
        ...
        while True:
            ...
            if self._agent_type == 'Cursor':
                if cursor_idx:
                    action = np.hstack(
                        [np.zeros_like(action[:6]), [flag[0]], action[:6], [flag[1], action[7]]]
                    )
                else:
                    action = np.hstack(
    ...
    def _get_cursor_pos(self, name=None):
        ...
        if self._agent_type in ['Sawyer', 'HDT', 'Baxter']:
            ...
            if name:
                ...
                return np.hstack([self._get_pos('cursor0'),
                                  self._get_pos('cursor1')])
        else:
            return None
    ...
    def _setup_action(self, action):
        ...
        if self._agent_type == 'Sawyer':
            ...
            action = np.concatenate([arm_action, gripper_right_action_actual,
                                    gripper_left_action_actual])
